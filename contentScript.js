        ...
        request.onreadystatechange = function() {
            if (request.readyState == 4){
                ...
                if (dueDate != null && assignmentName != null){
                    // console.log(dueDate + " 11:59pm");
                    if (new Date(dueDate + " 11:59 pm") - new Date() > 0){
                        assignmentInfo.push([assignmentName, dueDate[0], time, assignmentLink]);
                    }
...
document.getElementById("AssMan").addEventListener("click",function(){
        ...
        if (currentDisplay == htmlContent){
            ...
            var AMD = document.querySelectorAll(".AssManDiv");
            for (var i = 0; i < AMD.length; i++){
                AMD[i].style.margin = "40px";
            }

            var AMT = document.querySelectorAll(".AssManTable");
            for (var i = 0; i < AMT.length; i++){
                AMT[i].style.width = "100%";
            }
        }
