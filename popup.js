chrome.storage.sync.get(['finalAssignmentList', "courseInfo", "lastUpdatedTime", "assignedTask", "sortBy"], function (result) {
    ...
    if (sortBy == "course") {
        ...
        for (const course in finalAssignmentList) {
            ...
            s += "<div class='AssMan'>";
            s += `<p>Course Name: <a class='AssLink' href='https://blackboard.stonybrook.edu/webapps/blackboard/content/listContent.jsp?course_id=_${course}_1&handle=announcements_entry&mode=view' target='_blank'>` + courseInfo[course] + "</a></p>";
            ...
            for (var i = 0; i < finalAssignmentList[course].length; i++) {
                ...
                if (date - new Date() > 0){
                    s += "<tr><td class='AssManTd'><a class='AssLink' href='https://blackboard.stonybrook.edu/"+ finalAssignmentList[course][i][3] + "'  target='_blank'>" + finalAssignmentList[course][i][0].trim() + "</a></td><td class='AssManTd'>Due " + d + `  </td><td class='AssManTd'>${Math.floor(minutes / 60 / 24)}D ${Math.floor(minutes % (60 * 24) / 60)}H ${Math.floor(minutes % 60)}M</td>`;
                }
        ...
        if (Object.keys(extraAssignments).length != 0) {
            ...
            for (var i = 0; i < keys.length; i++) {
                ...
                if (displayDate.getHours() > 12) {
                    ...
                    dateStr += `${displayDate.getHours()}:${displayDate.getMinutes()} AM`;
                }

                if (displayDate - new Date() > 0){
    ...
    // sort by dueDate
            ...
            for (var i = 0; i < finalAssignmentList[course].length; i++) {
                ...
                if (date - (new Date) > 0){
                    let assignmentObj = {
                        ...
                        url: finalAssignmentList[course][i][3]
                    };
        ...
        if (assignments.length > 0){
            ...
            for (var i = 0; i < addedIndex.length; i++){
                ...
                var dateStr = weekdayShort[assignment.dateObj.getDay()] + ", " + (assignment.dateObj.getMonth()+1) + "/" + assignment.dateObj.getDate() + "/" + assignment.dateObj.getFullYear() + " ";
                ...
                if (assignment.url != undefined){
                    s += "<tr><td class='AssManTd'>" + `<a class='AssLink' href='https://blackboard.stonybrook.edu/webapps/blackboard/content/listContent.jsp?course_id=_${assignment.courseID}_1&handle=announcements_entry&mode=view' target='_blank'>` + assignment.course + "</a></td><td class='AssManTd'><a class='AssLink' href='https://blackboard.stonybrook.edu/"+ assignment.url + "'  target='_blank'>" + assignment.name + "</a></td><td class='AssManTd'>Due " + dateStr + `  </td><td class='AssManTd'>${Math.floor(minutes / 60 / 24)}D ${Math.floor(minutes % (60 * 24) / 60)}H ${Math.floor(minutes % 60)}M</td>`;
                }
                else{
    ...
    s += `</div>
        <div class='link'>
        <p><a class='ad-btn' href='index.html' target='_self'><button class='ad-btn'>add/delete more assignments</button>
        </a></p>`;
    ...
    var AssLinks = document.querySelectorAll(".AssLink")
    for (var s = 0; s < AssLinks.length; s++){
        AssLinks[s].style.color = "black";
    }
